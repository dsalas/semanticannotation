...
def describe_stanford_pos_tag(pos_tag):
    df = pd.read_csv("Stanford_POS_Tags.csv")
...
def getCategoriesDistance(question_data, categories):
    ...
    model = KeyedVectors.load_word2vec_format("./w2vmodel/SBW-vectors-300-min5.txt", binary=False)
...
def getCategory(question, categories):
    tagged_question = getTaggedQuestion(question)
